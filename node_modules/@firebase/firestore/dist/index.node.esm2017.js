...
class Firestore {
    ...
    constructor(databaseIdOrApp, authProvider, persistenceProvider = new MemoryComponentProvider()) {
...
class GrpcConnection {
    ...
    ensureActiveStub() {
        if (!this.cachedStub) {
            logDebug(LOG_TAG$c, 'Creating Firestore stub.');
    ...
    invokeRPC(rpcName, request, token) {
        ...
        return nodePromise((callback) => {
            logDebug(LOG_TAG$c, `RPC '${rpcName}' invoked with request:`, request);
            return stub[rpcName](request, metadata, (grpcError, value) => {
                if (grpcError) {
                    logDebug(LOG_TAG$c, `RPC '${rpcName}' failed with error:`, grpcError);
                    callback(new FirestoreError(mapCodeFromRpcCode(grpcError.code), grpcError.message));
                }
                else {
                    logDebug(LOG_TAG$c, `RPC '${rpcName}' completed with response:`, value);
    ...
    invokeStreamingRPC(rpcName, request, token) {
        ...
        logDebug(LOG_TAG$c, `RPC '${rpcName}' invoked (streaming) with request:`, request);
        const stub = this.ensureActiveStub();
        const metadata = createMetadata(this.databaseInfo, token);
        const stream = stub[rpcName](request, metadata);
        stream.on('data', (response) => {
            logDebug(LOG_TAG$c, `RPC ${rpcName} received result:`, response);
            results.push(response);
        });
        stream.on('end', () => {
            logDebug(LOG_TAG$c, `RPC '${rpcName}' completed.`);
    ...
    openStream(rpcName, token) {
        ...
        const stream = new StreamBridge({
            sendFn: (msg) => {
                if (!closed) {
                    logDebug(LOG_TAG$c, 'GRPC stream sending:', msg);
                    try {
                    ...
                    logDebug(LOG_TAG$c, 'Not sending because gRPC stream is closed:', msg);
                }
            },
            closeFn: () => {
                logDebug(LOG_TAG$c, 'GRPC stream closed locally via close().');
                close();
            }
        });
        grpcStream.on('data', (msg) => {
            if (!closed) {
                logDebug(LOG_TAG$c, 'GRPC stream received:', msg);
                stream.callOnMessage(msg);
            }
        });
        grpcStream.on('end', () => {
            logDebug(LOG_TAG$c, 'GRPC stream ended.');
            close();
        });
        grpcStream.on('error', (grpcError) => {
            logDebug(LOG_TAG$c, 'GRPC stream error. Code:', grpcError.code, 'Message:', grpcError.message);
            const code = mapCodeFromRpcCode(grpcError.code);
            close(new FirestoreError(code, grpcError.message));
        });
        logDebug(LOG_TAG$c, 'Opening GRPC stream');
